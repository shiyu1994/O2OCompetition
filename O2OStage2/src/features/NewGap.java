	...
	public static class Reducer extends ReducerBase { 
		...
		public void reduce(Record key, Iterator<Record> values, TaskContext context) throws IOException {
			...
			for(int i = 0; i < allReceivedDate.size(); ++i) {
				...
				counts.set(4, receivedPerMonth[month - 1]);	
				if(prevDateInt == -1) {	
					inBetween = 0.0;
					counts.set(5, null);
				}
				...
				Double getPrevious5Days = 0.0;
				Double getNext5Days = 0.0;
				...
				counts.set(8, getPrevious31Days);
				
				counts.set(9, getNext31Days);
				...
				counts.set(10, getPrevious15Days);
				counts.set(11, getNext15Days);
				
				counts.set(12, getPrevious15Days + getNext15Days);
				
				if(prevPrevDateInt == -1) {
					counts.set(13, null);
				}
				else {
					counts.set(13, (dateInt - prevPrevDateInt) * 1.0);
				}
				...
				if(nextNextDateInt == -1) {
					...
					counts.set(14, (nextNextDateInt - dateInt) * 1.0);
				}
				...
				if(nextGap != 1000) {
					counts.set(16, Math.sqrt(nextGap));
					counts.set(17, nextGap * nextGap * 1.0); 
				}
				else {
					counts.set(16, null);
					counts.set(17, null);
				...
				counts.set(18, Math.sqrt(receivedPerMonth[month - 1]));
				counts.set(19, receivedPerMonth[month - 1] * receivedPerMonth[month - 1]);	
				
				counts.set(20, inBetween); 
				
				counts.set(21, getPrevious5Days);
				counts.set(22, getNext5Days);
				
				if(nextDateInt != -1 && parseMonth(allReceivedDate.get(i + 1)) - month <= 1) {
					counts.set(23, (nextDateInt - dateInt) * 1.0);
				}
				else {
					counts.set(23, null);
				}
				
				if(prevDateInt != -1 && month - parseMonth(allReceivedDate.get(i - 1)) <= 1) {  
					counts.set(24, (dateInt - prevDateInt) * 1.0);
				}
				else {
					counts.set(24, null); 
				}
