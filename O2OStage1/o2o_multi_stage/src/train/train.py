...
def train_with_features(selected_features_file, num_round, cv=False):
    ...
    if cv:
        ...
        last_month = 7
    for i in xrange(2, last_month):
        col_names = np.genfromtxt(paths.my_path + 'X_{0}_{1}{2}_gap_col_names.csv'.format(i, 1, i - 1), delimiter=',', dtype=str)
        name_index = {}
        ...
        matrix = np.genfromtxt(paths.my_path + 'X_{0}_{1}{2}_gap.csv'.format(i, 1, i - 1), delimiter=',', dtype=str)
    ...
    col_names = np.genfromtxt(paths.my_path + 'X_{0}_{1}{2}_gap_col_names.csv'.format(last_month, 1, last_month - 1), delimiter=',', dtype=str)
    name_index = {}
    for j in xrange(col_names.shape[1]):
        name_index[col_names[0, j]] = int(col_names[1, j])
    selected_feature_indices = []
    for name in selected_features:
        selected_feature_indices.append(name_index[name])
    matrix = np.genfromtxt(paths.my_path + 'X_{0}_{1}{2}_gap.csv'.format(last_month, 1, last_month - 1), delimiter=',', dtype=str)
    ...
    if cv:
        ...
        cids_test[:] = matrix[:, name_index["cid"]]

    X_train_matrix = xgboost.DMatrix(X_train, y_train, missing=-9999)

    if cv:
        X_test_matrix = xgboost.DMatrix(X_test, y_test, missing=-9999)
    else:
        X_test_matrix = xgboost.DMatrix(X_test, missing=-9999)
...
if __name__ == '__main__':
    ...
    #cv()
    #train()
    train_with_features("features_no_cf.csv", 50, cv=True)
    pass
