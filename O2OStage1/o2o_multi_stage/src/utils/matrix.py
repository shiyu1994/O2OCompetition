...
class Matrix:
    def __init__(self, matrix, col_names, col_formats=["%s"], default=-9999):
    ...
    """
    def check_point(self, month, since, to, debug=False):
        if not debug:
            np.savetxt(paths.my_path + 'X_{0}_{1}{2}.csv'.format(month, since, to), self.matrix, fmt=",".join(self.col_formats))
            col_names_file = open(paths.my_path + 'col_names_{0}_{1}{2}.csv'.format(month, since, to), 'w')
        else:
            np.savetxt(paths.my_path + 'X_{0}_{1}{2}_debug.csv'.format(month, since, to), self.matrix, fmt=",".join(self.col_formats))
            col_names_file = open(paths.my_path + 'col_names_{0}_{1}{2}_debug.csv'.format(month, since, to), 'w')
    ...
    def join(self, key_name, feature_names, values, formats, dft=-9999):
        """
    ...
    def join_by_double_key(self, key_name0, key_name1, feature_name, values, fmt, dft=-9999):
        self.col_index.register(feature_name)
        self.matrix = np.hstack((self.matrix, np.zeros((self.ndata, 1))))
        self.col_formats.append(fmt)
        self.col_names.append(feature_name)
        for i in xrange(self.ndata):
            ...
            if values.has(key0):
                ...
                self.matrix[i, -1] = dft
    def gen_arith_feature(self, col0, col1, feature_name, op, fmt, dft=-9999):
        self.col_index.register(feature_name)
        self.matrix = np.hstack((self.matrix, np.zeros((self.ndata, 1))))
        self.col_formats.append(fmt)
        self.col_names.append(feature_name)
